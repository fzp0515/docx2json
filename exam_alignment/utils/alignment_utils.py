...
def extract_and_combine_numbers_in_not_start_by_number(str_val, number):
    ...
    pattern = regex.compile(
        r'(\d+)\\*[．.]|(\\*[（(])\d+\\*[）)]|\\*\d+\\\.|\d+、|\d+[ABCD] |\[(\d+)\]|\d+（|\d+【'.replace('\d+', number))
...
def longest_increasing_subsequence_index(topics):
    ...
    nums = [(extract_and_combine_numbers(topic), i) for i, topic in enumerate(topics) if
            extract_and_combine_numbers(topic) is not None]
